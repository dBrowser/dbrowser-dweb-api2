...
The internal implementation for [DBrowser](https://github.com/dbrowser/dbrowser)'s `DatArchive` APIs.
Works with DWeb 2.0.
...
### copy(srcArchive, srcName, dstArchive, dstName[, cb])
 ...
 - `srcArchive` Source DWebFs archive (object).
 - `srcName` Path to file or directory to copy (string).
 - `dstArchive` Destination DWebFs archive (object).
