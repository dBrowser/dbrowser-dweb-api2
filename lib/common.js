...
const {NotFoundError, EntryAlreadyExistsError, DestDirectoryNotEmpty, ArchiveNotWritableError} = require('dbrowser-error-messages')
...
function toBeakerError (err, info) {
  if (err.code == 16) {
    ...
    return new Error('The DDatabase daemon has reset due to internal error. Please try again.')
    ...
    console.error(`Pauls-DWeb-API: Unhandled error type from ${info}`, err)
